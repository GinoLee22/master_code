...
public class DList1 {
   ...
   */
    ...
    public void removeFront() {
        // Your solution here.
        if (head == null) {             // empty DList
            return;                     // jump out of the program and won't do "size--"
        }
        else if (head == tail) {        // DList with one element
            head = null;
            tail = null;
        }
        else {                          // DList with more than or equal to 2 elements
            head = head.next;
            head.prev = null;
        }
        size--;                         // NOTE: empty list won't reach hear because of return
    }
